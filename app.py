...
def display_research_results(state: ResearchState) -> None:
    ...
    with col1:
        ...
        if state.get("search_results"):
            with st.expander(f"🔍 Web Search Results ({len(state['search_results'])})", expanded=False):
        ...
        if state.get("scraped_content"):
            with st.expander(f"📄 Scraped Web Pages ({len(state['scraped_content'])})", expanded=False):
    ...
    with col2:
        ...
        if state.get("retrieved_chunks"):
            with st.expander(f"💡 Retrieved Context ({len(state['retrieved_chunks'])} chunks)", expanded=False):
                for i, chunk in enumerate(state["retrieved_chunks"][:5], 1):
                    metadata = chunk.get('metadata', {})
                    st.markdown(f"**Chunk {i}** (Similarity: {chunk.get('similarity_score', 0):.3f})")
...
def run_research(topic: str) -> Optional[ResearchState]:
    ...
    try:
        ...
        with st.spinner("Research in progress..."):
            # Run research
            final_state = st.session_state.agent.research(topic)
